...
**M3. Money as a first-class tile, in the same grid as products.** One cash gift (`שי`) sits beside the stroller, not in a separate tab. Guests send via Bit/Paybox; we do not sell gift cards. Israeli guests want to give money; forcing them to a secondary surface loses them. The design must also make an obviously nicer product tile than money tile, so money is the fallback and not the path of least resistance.
