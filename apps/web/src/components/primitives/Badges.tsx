import { PRIORITY_LABELS } from "@/lib/copy";
...
export function PriorityBadge({ priority }: { priority: Priority }) {
  const tones: Record<Priority, string> = {
    must: "bg-primary-tint text-primary",
    want: "bg-accent-tint text-accent-ink",
    nice: "bg-neutral-tint text-ink-muted",
  };
  return (
    <span
      className={`w-fit rounded-full px-2.5 py-0.5 text-micro font-medium ${tones[priority]}`}
    >
